...
async def get_current_user_query_token(
    token: str = Query(description="Bearer token (for file download endpoints)"),
    session: AsyncSession = Depends(get_session),
) -> User:
    """
    Same as get_current_user but reads token from ?token= query param.
    Used for file download endpoints where the browser cannot send an
    Authorization header (e.g. <a href=...> triggered downloads).
    """
    credentials_error = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail="Could not validate credentials",
